...
class cnn_ocr:
    ...
    def __init__(self):
        ...
        self.y = tf.placeholder(tf.float32, [None,12,self.num_class])
    ...
    def network(self, x):
        ...
        predict = tf.reshape(predict, [-1, 1, self.num_class])
        for i in range(11):
            pred=slim.fully_connected(fc3, self.num_class,activation_fn=tf.nn.softmax,scope='pr'+str(i+1))
            pred=tf.reshape(pred,[-1,1,self.num_class])
            predict=tf.concat([predict,pred],1)
